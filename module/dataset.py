...
class Dataset:
    def __init__(self, stock_csv_paths, window = 30, late = 1, train_rate = 0.8) -> None:
        ...
        self.test_start_idx = int(self.raw_all.shape[1] * train_rate)
    ...
    def generate(self, window = 30, late = 1):
        ...
        while target_idx < self.raw_all.shape[1]:
            end = target_idx - late - 1
            start = end - window + 1
            X_test.append(self.raw_all[:, start:end])
            Y_test.append(self.raw_all[:, target_idx].T)
            target_idx += 1
        return np.nan_to_num(np.array(X_train)), np.nan_to_num(np.array(Y_train)), np.nan_to_num(np.array(X_test)), np.nan_to_num(np.array(Y_test))
